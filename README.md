# Know Nepal Documentation

Public documentation for the **Know Nepal** project.

Know Nepal is a digital knowledge platform focused on Nepal, providing
structured information across different domains and allowing users to
explore destinations and create personalized itineraries with Know Nepal AI.

## Documentation

### About

- [Project Overview](about/overview.md)
- [Vision](about/vision.md)
- [Project Scope](about/scope.md)

### Architecture

- [Architecture Overview](architecture/overview.md)
- [System Context](architecture/system-context.md)
- [Backend Architecture](architecture/backend.md)
- [Data Architecture](architecture/data.md)
- [Request Lifecycle](architecture/request-lifecycle.md)

### Domains

Documentation for the knowledge domains supported by Know Nepal.

- Geography
- Education
- Destinations
- Culture
- History
- Wildlife
- Healthcare

### Development

- [Development Setup](development/setup.md)
- [Repository Structure](development/repository-structure.md)
- [Testing](development/testing.md)
- [Development Conventions](development/conventions.md)

### Security

- [Security Overview](security/overview.md)
- [Authentication](security/authentication.md)

### Architecture Decisions

- [Architecture Decision Records](decisions/README.md)

### Contributing

- [Contribution Workflow](contributing/workflow.md)
- [Documentation Guidelines](contributing/documentation.md)

### Project Status

- [Current Status](status/current-state.md)
- [Roadmap](status/roadmap.md)

## Documentation Principles

The documentation follows a few simple principles:

- Document the system as it currently exists.
- Keep implementation details in the source repositories.
- Clearly distinguish implemented, in-progress, planned, and historical work.
- Avoid documenting secrets, credentials, or private configuration.
- Prefer accurate technical information over marketing language.
- Update documentation when significant architectural decisions change.

## Repository Structure

The Know Nepal project is organized across multiple repositories.

The core application is maintained separately from this public
documentation repository.

This repository focuses on documenting the project's architecture,
development practices, system concepts, and other information useful
to developers and contributors.

## Source of Truth

The actual implementation and configuration remain the source of truth
for the current state of the system.

Documentation should describe that implementation rather than define
functionality that does not yet exist.

## Project

**Know Nepal**

A project focused on making structured knowledge about Nepal easier
to explore and use.

---

**Status:** Active Development
